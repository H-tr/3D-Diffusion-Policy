...
class KortexRunner(BaseRunner):
    def __init__(self,
        ...
        def env_fn(is_test=True):
            return MultiStepWrapper(
                SimpleVideoRecordingWrapper(KortexEnv(
                    task_name=task_name,
    ...
    def run(self, policy: BasePolicy):
        ...
        ##############################
        # train env loop
        for episode_id in tqdm.tqdm(range(self.episode_train), desc=f"Kortex {self.task_name} Train Env",leave=False, mininterval=self.tqdm_interval_sec):
            # start rollout
            obs = env_train.reset()
